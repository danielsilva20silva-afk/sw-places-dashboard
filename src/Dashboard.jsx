    ...
    <div ref={ref} style={{ position: "relative", display: "inline-block" }}>
      ...
      {open && (
        <div style={{
          ...
          border: "1px solid #F0F0F0", overflow: "hidden", minWidth: 175,
        }}>
          {STATUSES.map(s => (
            ...
            >
              ...
              {s}
              {s === status && <span style={{ marginLeft: "auto", fontSize: 12, color: "#C9A96E" }}>✓</span>}
            </button>
          ))}
        </div>
      )}
  ...
  return (
    <div style={{ display: "flex", gap: 6 }}>
      <a href={`tel:${lead.phone}`} onClick={e => e.stopPropagation()} title="Ligar" style={{
        ...
        textDecoration: "none", fontSize: 15, border: "1px solid #BBF7D0", flexShrink: 0,
      }}>📞</a>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", alignItems: "flex-end", justifyContent: "flex-end" }} onClick={onClose}>
      ...
      <div style={{
        ...
        height: "100vh", overflowY: "auto", boxShadow: "-8px 0 40px rgba(0,0,0,0.12)",
        ...
        <div style={{ padding: "20px 24px 16px", borderBottom: "1px solid #F0F0F0", position: "sticky", top: 0, background: "white", zIndex: 10 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
            ...
            <span style={{ fontSize: 12, color: "#888" }}>{relDate(lead.date)}</span>
          </div>
        ...
        <div style={{ padding: "20px 24px", flex: 1, display: "flex", flexDirection: "column", gap: 20 }}>
          <div style={{ display: "flex", gap: 10 }}>
            <a href={`tel:${lead.phone}`} style={{
              ...
              background: "#16A34A", color: "white", borderRadius: 12, padding: "12px",
              textDecoration: "none", fontSize: 13, fontWeight: 600,
            }}>📞 Ligar</a>
            <a href={`https://wa.me/${lead.phone.replace(/[^0-9]/g, "")}`} target="_blank" rel="noreferrer" style={{
              flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
              background: "#25D366", color: "white", borderRadius: 12, padding: "12px",
              textDecoration: "none", fontSize: 13, fontWeight: 600,
            }}>💬 WhatsApp</a>
          </div>
          ...
          <div>
            <p style={{ fontSize: 11, color: "#888", textTransform: "uppercase", letterSpacing: "0.5px", margin: "0 0 10px" }}>Estado</p>
              ...
              {STATUSES.map(s => {
                ...
                return (
                  <button key={s} onClick={() => setStatus(s)} style={{
                    ...
                    background: active ? c.bg : "white", color: active ? c.text : "#555",
          ...
          <div>
            <p style={{ fontSize: 11, color: "#888", textTransform: "uppercase", letterSpacing: "0.5px", margin: "0 0 8px" }}>Notas</p>
            <textarea value={notes} onChange={e => setNotes(e.target.value)}
              placeholder="Notas sobre este lead..." rows={4} style={{
                width: "100%", border: "1px solid #E5E5E5", borderRadius: 10,
                padding: "12px 14px", fontSize: 13, color: "#111", resize: "none",
        ...
        <div style={{ padding: "16px 24px", borderTop: "1px solid #F0F0F0", display: "flex", gap: 10 }}>
          <button onClick={() => { onUpdate(lead.id, status, notes); onClose(); }} style={{
            flex: 1, background: "#111", color: "white", border: "none",
            borderRadius: 12, padding: "13px", fontSize: 14, fontWeight: 600, cursor: "pointer",
          }}>Guardar</button>
          <button onClick={onClose} style={{
            padding: "13px 18px", border: "1px solid #E5E5E5", borderRadius: 12,
            fontSize: 14, color: "#555", background: "white", cursor: "pointer",
          }}>Cancelar</button>
        </div>
      </div>
    </div>
...
function NewsletterTab({ leads }) {
  ...
  const count = segments.length === 0 ? leads.length
    : leads.filter(l => segments.some(s => l.intention === s || l.budget === s)).length;
    ...
    <div style={{ maxWidth: 680, margin: "0 auto" }}>
      <div style={{ background: "white", borderRadius: 20, border: "1px solid #EBEBEB", overflow: "hidden" }}>
        ...
        <div style={{ padding: "24px 28px", display: "flex", flexDirection: "column", gap: 22 }}>
          <div>
            <p style={{ fontSize: 11, color: "#888", textTransform: "uppercase", letterSpacing: "0.5px", margin: "0 0 10px" }}>Enviar para {segments.length === 0 ? "(todos)" : ""}</p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {allSegments.map(s => (
                <button key={s} onClick={() => toggle(s)} style={{
                  padding: "6px 14px", borderRadius: 20, fontSize: 12, fontWeight: 500,
                  border: `1.5px solid ${segments.includes(s) ? "#C9A96E" : "#E5E5E5"}`,
                  background: segments.includes(s) ? "#FDF8F0" : "white",
                  color: segments.includes(s) ? "#92400E" : "#555", cursor: "pointer",
                }}>{s}</button>
              ))}
            </div>
          </div>
          <div style={{ background: "#F8F7F4", borderRadius: 14, padding: "16px 18px" }}>
            <p style={{ fontSize: 11, color: "#888", textTransform: "uppercase", letterSpacing: "0.5px", margin: "0 0 10px" }}>✨ Gerar com IA</p>
            <div style={{ display: "flex", gap: 10 }}>
              <input value={topic} onChange={e => setTopic(e.target.value)} onKeyDown={e => e.key === "Enter" && generate()}
                placeholder="Ex: nova propriedade off-market em Aljezur..." style={{
                  flex: 1, border: "1px solid #E5E5E5", borderRadius: 10, padding: "10px 14px",
                  fontSize: 13, outline: "none", color: "#111", background: "white", fontFamily: "inherit",
                }} />
              <button onClick={generate} disabled={loading || !topic} style={{
                padding: "10px 18px", borderRadius: 10, border: "none",
                background: topic && !loading ? "#C9A96E" : "#E5E5E5",
                color: topic && !loading ? "white" : "#999",
                fontSize: 13, fontWeight: 600, cursor: topic && !loading ? "pointer" : "not-allowed", whiteSpace: "nowrap",
              }}>{loading ? "A gerar..." : "Gerar"}</button>
            </div>
          </div>
          <div>
            <p style={{ fontSize: 11, color: "#888", textTransform: "uppercase", letterSpacing: "0.5px", margin: "0 0 8px" }}>Assunto</p>
            <input value={subject} onChange={e => setSubject(e.target.value)}
              placeholder="Ex: Nova oportunidade off-market — Junho 2026" style={{
                width: "100%", border: "1px solid #E5E5E5", borderRadius: 10, padding: "10px 14px",
                fontSize: 13, outline: "none", color: "#111", boxSizing: "border-box", fontFamily: "inherit",
              }} />
          </div>
          <div>
            <p style={{ fontSize: 11, color: "#888", textTransform: "uppercase", letterSpacing: "0.5px", margin: "0 0 8px" }}>Mensagem</p>
            <textarea value={body} onChange={e => setBody(e.target.value)}
              placeholder="Escreve aqui ou gera com IA acima..." rows={10} style={{
                width: "100%", border: "1px solid #E5E5E5", borderRadius: 10, padding: "12px 14px",
                fontSize: 13, outline: "none", color: "#111", resize: "vertical", lineHeight: 1.7,
          ...
          {sent && (
            <div style={{ background: "#F0FDF4", border: "1px solid #BBF7D0", borderRadius: 12, padding: "12px 16px", fontSize: 13, color: "#15803D", fontWeight: 500 }}>
              ✅ Newsletter enviada para {count} lead{count !== 1 ? "s" : ""}!
            </div>
          )}
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button style={{
              padding: "13px 20px", border: "1px solid #E5E5E5", borderRadius: 12,
              fontSize: 14, color: "#555", background: "white", cursor: "pointer",
...
export default function Dashboard({ user, onLogout }) {
  ...
  const stats = [
    { label: "Total", value: leads.length, color: "#111" },
    { label: "Novos", value: leads.filter(l => l.status === "Novo").length, color: "#2563EB" },
    { label: "Em contacto", value: leads.filter(l => l.status === "Contactado").length, color: "#D97706" },
    { label: "Fechados", value: leads.filter(l => l.status === "Fechado").length, color: "#16A34A" },
  ];

  const FilterLabel = ({ children }) => (
    <p style={{ fontSize: 10, color: "#999", textTransform: "uppercase", letterSpacing: "0.5px", fontWeight: 600, margin: "0 0 5px", paddingLeft: 2 }}>{children}</p>
  );

  return (
    <div style={{ fontFamily: "'Inter', -apple-system, sans-serif", background: "#F8F7F4", minHeight: "100vh" }}>
      {/* Header */}
      <div style={{ background: "#111", position: "sticky", top: 0, zIndex: 40 }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 16px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 54 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 28, height: 28, background: "#C9A96E", borderRadius: 7, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, color: "white" }}>S</div>
            <span style={{ fontSize: 14, fontWeight: 600, color: "white", letterSpacing: "-0.3px" }}>SW Places</span>
          </div>
          <div style={{ display: "flex", gap: 2 }}>
            {[["leads", "Leads"], ["newsletter", "Newsletter"]].map(([tab, label]) => (
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "24px 16px 48px" }}>
        {activeTab === "leads" && (
          <>
            {/* Stats */}
            <div style={{ display: "flex", gap: 12, marginBottom: 24, overflowX: "auto", paddingBottom: 2 }}>
              {stats.map(s => (
                <div key={s.label} style={{ background: "white", borderRadius: 14, padding: "16px 20px", border: "1px solid #EBEBEB", flexShrink: 0, minWidth: 110 }}>
                  <p style={{ fontSize: 11, color: "#888", textTransform: "uppercase", letterSpacing: "0.5px", margin: "0 0 6px" }}>{s.label}</p>
                  <p style={{ fontSize: 30, fontWeight: 700, color: s.color, letterSpacing: "-1px", margin: 0, lineHeight: 1 }}>{s.value}</p>
                </div>
              ))}
            </div>

            {/* Search + Filters — with labels */}
            <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap", alignItems: "flex-end" }}>
              <div style={{ flex: 1, minWidth: 180 }}>
                <FilterLabel>Pesquisar</FilterLabel>
                <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Nome ou email..." style={{
                  width: "100%", border: "1px solid #E5E5E5", borderRadius: 10, padding: "9px 14px",
                  fontSize: 13, outline: "none", color: "#111", background: "white",
              ...
              {[
                ...
                <div key={i}>
                  ...
                  <select value={f.value} onChange={e => f.set(e.target.value)} style={{
                    border: "1px solid #E5E5E5", borderRadius: 10, padding: "9px 32px 9px 12px",
                    fontSize: 13, color: "#111", background: "white", cursor: "pointer", outline: "none",
                  }}>
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
                      <span style={{ fontSize: 14, fontWeight: 600, color: "#111", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                        {lead.name}
                      </span>
                      {lead.notes && (
                        <span title={lead.notes} style={{ fontSize: 11, color: "#C9A96E", cursor: "default" }}>📝</span>
                      )}
                    </div>
